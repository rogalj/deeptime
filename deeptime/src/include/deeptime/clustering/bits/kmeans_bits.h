...
inline std::tuple<np_array<T>, np_array<int>> cluster(const np_array_nfc<T> &np_chunk,
    ...
    if (n_threads == 0) {
        for (py::ssize_t i = 0; i < n_frames; ++i) {
            int argMinDist = 0;
            {
                T minDist = Metric::template compute<T>(&chunk(i, 0), &centers(0, 0), dim);
                for (std::size_t j = 1; j < n_centers; ++j) {
                    auto dist = Metric::template compute<T>(&chunk(i, 0), &centers(j, 0), dim);
                    if (dist < minDist) {
                        minDist = dist;
                        argMinDist = j;
                    }
...
#pragma omp parallel for schedule(static, 1)
        for (py::ssize_t i = 0; i < n_frames; ++i) {
            ...
            for (std::size_t j = 0; j < n_centers; ++j) {
                dists[j] = Metric::template compute<T>(&chunk(i, 0), &centers(j, 0), dim);
            }
...
inline T costFunction(const np_array_nfc<T> &np_data, const np_array_nfc<T> &np_centers,
    ...
    for (std::size_t i = 0; i < n_frames; i++) {
        auto l = Metric::template compute<T>(&data(i, 0), &centers(assignmentsPtr[i], 0), dim);
        {
            value += l * l;
        }
    }
